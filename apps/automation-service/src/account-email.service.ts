...
export class AccountEmailService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(private readonly db: DatabaseProvider, config: ConfigService) {
    if (process.env.NODE_ENV === 'production') throw new Error('Automation gRPC/AMQP/SMTP transport requires TLS in production');
    this.identity = new IdentityClient(config.getOrThrow<string>('IDENTITY_GRPC_TARGET'), 'Automation', config.getOrThrow<string>('AUTOMATION_IDENTITY_SECRET'));
    this.smtp = createTransport({ host: config.getOrThrow<string>('SMTP_HOST'), port: Number(config.getOrThrow<string>('SMTP_PORT')), secure: false, connectionTimeout: 2000, socketTimeout: 5000 });
  ...
  private async connectBroker() {
    ...
    try {
      ...
      await channel.consume(queue, message => { if (message) void this.consume(channel, message); }, { noAck: false });
  ...
  private async consume(channel: Channel, message: ConsumeMessage) {
    ...
    try {
      ...
      if (event.event_type !== 'AccountEmailRequested' || event.producer !== 'Identity') throw new Error('UNSUPPORTED_EVENT');
    } catch {
    ...
    try {
      await this.db.withTransaction(async (client: PoolClient) => {
        const inbox = await client.query(`INSERT INTO inbox_events(consumer_name,event_id,event_type,aggregate_id,aggregate_version)
        ...
        const notification = await client.query<{ id: string }>(`INSERT INTO notifications(user_id,type,title,body,event_id,status,dedupe_key)
        ...
        await client.query(`INSERT INTO notification_deliveries(notification_id,channel,delivery_key,private_context)
          VALUES($1,'EMAIL',$2,$3) ON CONFLICT (delivery_key) DO NOTHING`,
        [notification.rows[0]!.id, `account-email:${tokenId}`, JSON.stringify({ token_id: tokenId })]);
      });
      channel.ack(message);
    } catch {
  ...
  private async sendPending() {
    ...
    this.sending = true;
    try {
      const deliveries = await this.db.query<Delivery>(`UPDATE notification_deliveries SET status='SENDING',claim_owner=$1,
        ...
        WHERE id IN (SELECT id FROM notification_deliveries WHERE channel='EMAIL' AND next_attempt_at<=now()
          ...
          ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 5) RETURNING id,attempt_count,private_context`, [this.owner]);
  ...
  private async deliver(delivery: Delivery) {
    try {
      ...
      const response = await this.identity.getAccountEmailDelivery({ token_id: delivery.private_context.token_id });
      if (!response.valid || !response.delivery) {
        await this.db.query("UPDATE notification_deliveries SET status='FAILED',last_error='TOKEN_INVALID',private_context='{}'::jsonb,claim_owner=NULL,claim_until=NULL,updated_at=now() WHERE id=$1 AND claim_owner=$2", [delivery.id, this.owner]);
        return;
      }
      const info = await this.smtp.sendMail({
        from: 'Wolfari <no-reply@wolfari.local>',
        to: response.delivery.recipient_email,
        subject: response.delivery.purpose === 1 ? 'Xác minh email Wolfari' : 'Đặt lại mật khẩu Wolfari',
        text: `Mở liên kết sau để tiếp tục: ${response.delivery.one_time_link}\nNếu bạn không yêu cầu, hãy bỏ qua email này.`,
      });
      await this.db.query(`UPDATE notification_deliveries SET status='SENT',provider_reference=$3,sent_at=now(),
        claim_owner=NULL,claim_until=NULL,last_error=NULL,private_context='{}'::jsonb,updated_at=now()
        WHERE id=$1 AND claim_owner=$2`, [delivery.id, this.owner, String(info.messageId).slice(0, 180)]);
    } catch {
      const attempt = delivery.attempt_count + 1;
      const delay = retryDelays[Math.min(attempt - 1, retryDelays.length - 1)] ?? 900000;
      await this.db.query(`UPDATE notification_deliveries SET status=$3,attempt_count=$4,
      ...
      [delivery.id, this.owner, attempt > retryDelays.length ? 'FAILED' : 'PENDING', attempt, delay]).catch(() => {});
    }
  }

  async diagnostics() {
    const rows = await this.db.query<{ status: string; count: string }>("SELECT status,count(*)::text AS count FROM notification_deliveries WHERE channel='EMAIL' AND status<>'SENT' GROUP BY status");
    let smtp: 'up' | 'down' = 'down';
    try { if (await this.smtp.verify()) smtp = 'up'; } catch { /* independent of login readiness */ }
    return { broker: this.channel ? 'up' : 'down', smtp, deliveries: Object.fromEntries(rows.rows.map(row => [row.status.toLowerCase(), Number(row.count)])) };
  }
}
...
export class AccountEmailDependenciesController {
  ...
  dependencies() { return this.delivery.diagnostics(); }
}
